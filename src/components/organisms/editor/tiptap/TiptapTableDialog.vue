<template>
	<Dialog :show="show" @close="closeDialog" title="Tambah Tabel">
		<form @submit.prevent="onSubmit">
			<div class="flex flex-col space-y-5">
				<div class="flex flex-row space-x-5">
					<InputContainer class="w-full flex-1">
						<Label for="input-table-columns">Kolom</Label>
						<Input v-model="inputColumnsRef" id="input-table-columns" class="w-full" min="1" required type="number" />
					</InputContainer>
					<InputContainer class="w-full flex-1">
						<Label for="input-table-rows">Baris</Label>
						<Input v-model="inputRowsRef" id="input-table-rows" class="w-full" min="1" required type="number" />
					</InputContainer>
				</div>
				<SwitchGroup>
				...
				<div class="flex flex-row justify-end space-x-3">
					...
					<button class="rounded-md bg-blue-700 px-4 py-3 text-sm font-medium text-white hover:bg-opacity-80" type="submit">Tambah</button>
				</div>
			</div>
		</form>
	</Dialog>
